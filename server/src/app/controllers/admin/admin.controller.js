...
export async function totalAdmin(req, res) {
    return responseSuccess(res, await adminService.getTotalAdmins(req.query));
}

export async function readRoot(req, res) {
    return responseSuccess(res, await adminService.filter(req.query));
}

export async function readItem(req, res) {
    await responseSuccess(res, await adminService.details(req.params.id));
}
